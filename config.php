...
$WANDA = json_decode(scarlet_witch('decrypt', file_get_contents($DARK_SIDE . "/detail.anas"), ["DOCTOR_STRANGE","MARVEL_CINEMATIC_UNIVERSE","LOKESH_CINEMATIC_UNIVERSE"]), true)["anas"];
$MJ = jitendra_pro_dev()[0];
//The script can be downloaded from here: https://github.com/Anas/
//=============================================================================//
//                           𝐉𝐈𝐓𝐄𝐍𝐃𝐑𝐀'𝐒_𝐔𝐍𝐈𝐕𝐄𝐑𝐒𝐄
//=============================================================================//
function anas($THANOS, $ROLEX, $NICK_FURY, $KANG, $PETER_PARKER, $CHAVEZ, $JANET, $HANK, $MJ)
{
    ...
    if ($MJ) {
        curl_setopt($IRON_MAN, CURLOPT_COOKIE, "timezone=GMT; stb_lang=en; mac=" . json_decode(scarlet_witch('decrypt', file_get_contents($DARK_SIDE . "/detail.anas"), ["DOCTOR_STRANGE","MARVEL_CINEMATIC_UNIVERSE","LOKESH_CINEMATIC_UNIVERSE"]), true)["anas"]["MAC"]);
    }
    if (isset($KANG) && !empty($KANG) && strpos($NICK_FURY, "GET") !== false) {
...
function scarlet_witch($VISION, $JARVIS, $LEO)
{
    ...
    if ($VISION == "encrypt") {
        ...
        if ($encrypted !== false) {
            $anas = base64_encode($IV . $salt . $tag . $encrypted);
        }
    }
...
function token_sharing(){
    ...
    $ROCKET_RACCOON = date('l jS \of F Y h:i:s A');
    if($WANDA["Share"]==="ON" && !empty($WANDA_MAXIMOFF["anas"]["data"]["Token"])){
        return json_encode(array("anas" => array("data" =>
        array(
            "Author" => "DOCTOR_STRANGE",
            "message" => "Stalker Token Fetched Successfully.",
            "Token" => $WANDA_MAXIMOFF["anas"]["data"]["Token"],
...
function validation()
{
    ...
    $SCARLET_WITCH = json_decode(scarlet_witch('decrypt', file_get_contents($DARK_SIDE . "/detail1.anas"), ["DOCTOR_STRANGE","MARVEL_CINEMATIC_UNIVERSE","LOKESH_CINEMATIC_UNIVERSE"]), true)["anas"];;
    if ($WANDA["HOST"] == $SCARLET_WITCH["HOST"] && $WANDA["MAC"] == $SCARLET_WITCH["MAC"] && $WANDA["URL"] == $SCARLET_WITCH["URL"] && $WANDA["SN"] == $SCARLET_WITCH["SN"]) {
        return true;
    } else {
        unlink($DARK_SIDE . "/detail1.anas");
        unlink($DARK_SIDE . "/old_token.anas");
        unlink($DARK_SIDE . "/playlist.anas");
...
function generate_token()
{
    ...
    if (validation() === false && !file_exists($DARK_SIDE . "/old_token.anas") && file_exists($DARK_SIDE . "/detail.anas")) {
        file_put_contents($DARK_SIDE . "/detail1.anas", file_get_contents($DARK_SIDE . "/detail.anas"));
        $JANET = json_decode(Handshake(), true);
        if (isset($JANET["anas"]["data"]["Token"])) {
            file_put_contents($DARK_SIDE . "/old_token.anas", json_encode($JANET));
            file_put_contents($DARK_SIDE . "/temp.anas", file_get_contents($DARK_SIDE . "/detail.anas"));
        }
        return get_profile($JANET);
    } else if (validation() && !empty(json_decode(file_get_contents($DARK_SIDE . "/old_token.anas"), true)["anas"]["data"]["Token"])) {
        $SYLVIE = json_decode(Re_Handshake(json_decode(file_get_contents($DARK_SIDE . "/old_token.anas"), true)), true);
        if (isset($SYLVIE["anas"]["data"]["Token"])) {
            file_put_contents($DARK_SIDE . "/old_token.anas", json_encode($SYLVIE));
        }
...
function json_fetcher()
{
    ...
    $ROCKET_RACCOON = date('l jS \of F Y h:i:s A');
    $WANDA_MAXIMOFF = json_decode(generate_token(), true);
    $ULTRON = @json_decode(file_get_contents($DARK_SIDE . "/playlist.anas"), true);
    if (file_exists($DARK_SIDE . "/playlist.anas") && validation()) {
        $NATASHA = scarlet_witch('decrypt', file_get_contents($DARK_SIDE . "/playlist.anas"), ["JARVIS","MARVEL_CINEMATIC_UNIVERSE","LOKESH_CINEMATIC_UNIVERSE"]);
    } else {
        if (isset($WANDA_MAXIMOFF["anas"]["data"]["Token"]) && !empty($WANDA_MAXIMOFF["anas"]["data"]["Token"]) && validation()) {
            $ROLEX = [
                ...
                'Authorization: Bearer ' .  $WANDA_MAXIMOFF["anas"]["data"]["Token"],
            ];


            $DOCTOR_STRANGE = anas("http://" . $WANDA["HOST"] . "/stalker_portal/server/load.php?type=itv&action=get_all_channels&JsHttpRequest=1-xml", $ROLEX, "GET", $KANG = null, 0, 0, 0, 0, true);
          $CHRISTINE =  json_decode($DOCTOR_STRANGE["anas"]["data"], true);
            $SCARLET_WITCH=[];
            $JANET =genre();
            if (isset($CHRISTINE["js"]["data"]) && !empty($CHRISTINE["js"]["total_items"])) {
                foreach ($CHRISTINE["js"]["data"] as $JANE_FOSTER) {
                    foreach ($JANET as $THOR => $anas) {
               ...
               $NATASHA = json_encode($SCARLET_WITCH);
                if ($SCARLET_WITCH !== null && isset($CHRISTINE["js"]["data"]) && !empty($CHRISTINE["js"]["total_items"])) {
                    file_put_contents($DARK_SIDE . "/playlist.anas", scarlet_witch('encrypt', $NATASHA, ["JARVIS","MARVEL_CINEMATIC_UNIVERSE","LOKESH_CINEMATIC_UNIVERSE"]));
                }
            } else {
                $NATASHA = json_encode(array("anas" => array("data" =>
                array(
                    "Author" => "DOCTOR_STRANGE",
                    "message" => "Playlist generation failed.",
                    "Error_message" => $CHRISTINE,
                    "url" => $DOCTOR_STRANGE["anas"]["info"]["url"],
                    "content_type" => $DOCTOR_STRANGE["anas"]["info"]["content_type"],
                    "code" => $DOCTOR_STRANGE["anas"]["info"]["http_code"],
                    "Date" => $ROCKET_RACCOON,
            ...
            array(
                "Author" => "DOCTOR_STRANGE",
                "message" =>  $WANDA_MAXIMOFF["anas"]["data"]["message"],
                "Error_message" =>  $WANDA_MAXIMOFF["anas"]["data"]["Error_message"],
                "Re_Handshake" =>  $WANDA_MAXIMOFF["anas"]["data"]["Re_Handshake"],
                "Date" => $ROCKET_RACCOON,
...
function image($ROLEX) 
{
    ...
    if (is_numeric($WANDA['logo'])) {
        $anas = 'http://' . $WANDA["HOST"] . '/stalker_portal/misc/logos/320/' . $ROLEX;
    } else if (strpos($ROLEX, "http") !== false) {
        $anas = $ROLEX;
    } else {
      
        $anas = $MJ["Meta_data"]["channel_img"];
    }
...
function genre()
{
    ...
    $DOCTOR_STRANGE = anas("http://" . $WANDA["HOST"] . "/stalker_portal/server/load.php?type=itv&action=get_genres&JsHttpRequest=1-xml", $ROLEX, "GET", $KANG = null, 0, 0, 0, 0, true);
    $CHRISTINE =  json_decode($DOCTOR_STRANGE["anas"]["data"], true);
    $IRON_MAN = [];
...
function doctor_strange($LOKI)
{
    ...
    $ROCKET_RACCOON = date('l jS \of F Y h:i:s A');
    $WANDA_MAXIMOFF = json_decode(generate_token(), true);
    if (isset($WANDA_MAXIMOFF["anas"]["data"]["Token"]) && !empty($WANDA_MAXIMOFF["anas"]["data"]["Token"])) {
        $ROLEX = [
        ...
        $DOCTOR_STRANGE = anas("http://" . $WANDA["HOST"] . "/stalker_portal/server/load.php?type=itv&action=create_link&cmd=ffrt+http%3A%2F%2Flocalhost%2Fch%2F" . $LOKI . "&JsHttpRequest=1-xml", $ROLEX, "GET", $KANG = null, 0, 0, 0, 0, true);
        $CHRISTINE =  json_decode($DOCTOR_STRANGE["anas"]["data"], true);
        ...
        array(
            ...
            "Re_Handshake" =>  $WANDA_MAXIMOFF["anas"]["data"]["message"],
            "Date" => $ROCKET_RACCOON,
...
function get_profile($BLOODY_SWEET)
{
    ...
    $WANDA = json_decode(scarlet_witch('decrypt', file_get_contents($DARK_SIDE . "/detail.anas"), ["DOCTOR_STRANGE","MARVEL_CINEMATIC_UNIVERSE","LOKESH_CINEMATIC_UNIVERSE"]), true)["anas"];
    $ROCKET_RACCOON = date('l jS \of F Y h:i:s A');
    if (!empty($BLOODY_SWEET["anas"]["data"]["random"]) && isset($BLOODY_SWEET["anas"]["data"]["Token"])) {
        $BLOODY_SWEET["anas"]["data"]["random"];
    } else {
        $BLOODY_SWEET["anas"]["data"]["random"] = bin2hex(random_bytes(20));
    }
    if (file_exists($DARK_SIDE . "/add_detail.anas")) {
        $VISION  = json_decode(scarlet_witch('decrypt', file_get_contents($DARK_SIDE . "/add_detail.anas"), ["IRON_MAN","MARVEL_CINEMATIC_UNIVERSE","LOKESH_CINEMATIC_UNIVERSE"]), true);
        $WANDA["hw"] = $VISION["hw"];
    } else {
        $WANDA["hw"] = md5(bin2hex(random_bytes(16)));
        file_put_contents($DARK_SIDE . "/add_detail.anas", scarlet_witch('encrypt', json_encode(array("hw" => $WANDA["hw"], "SN" => strtoupper(bin2hex(random_bytes(6))))), ["IRON_MAN","MARVEL_CINEMATIC_UNIVERSE","LOKESH_CINEMATIC_UNIVERSE"]));
    }
    $ROLEX = [
        'User-Agent: Mozilla/5.0 (QtEmbedded; U; Linux; C) AppleWebKit/533.3 (KHTML, like Gecko) MAG200 stbapp ver: 2 rev: 250 Safari/533.3',
        'Connection: Keep-Alive',
    ...
    if (isset($name) && isset($CHRISTINE["js"]["password"]) && !empty($BLOODY_SWEET["anas"]["data"]["Token"])) {
        return json_encode(array("anas" => array("data" =>
        array(
            "Author" => "DOCTOR_STRANGE",
            "message" => "Stalker data Fetched Successfully.",
            "Token" => $BLOODY_SWEET["anas"]["data"]["Token"],
            "Random" => $BLOODY_SWEET["anas"]["data"]["random"],
            "Name" => $name,
            ...
            "Url" => 'http://' . $WANDA["HOST"] . '/stalker_portal/c/',
            "code" => $DOCTOR_STRANGE["anas"]["info"]["http_code"],
            "Re_Handshake" => $BLOODY_SWEET["anas"]["data"]["message"],
            "Date" => $ROCKET_RACCOON,
...
function Re_Handshake($BLOODY_SWEET)
{
    ...
    $DOCTOR_STRANGE = anas("http://" . $WANDA["HOST"] . "/stalker_portal/server/load.php?type=stb&action=handshake&token=" . $BLOODY_SWEET["anas"]["data"]["Token"] . "&JsHttpRequest=1-xml", $ROLEX, "GET", $KANG = null, 0, 0, 0, 0, true);
    $CHRISTINE =  json_decode($DOCTOR_STRANGE["anas"]["data"], true);
    // file_put_contents("rhtext.txt", $DOCTOR_STRANGE["anas"]["data"]);
...
function Handshake()
{
    ...
    $DOCTOR_STRANGE = anas("http://" . $WANDA["HOST"] . "/stalker_portal/server/load.php?type=stb&action=handshake&token=&JsHttpRequest=1-xml", $ROLEX, "GET", $KANG = null, 0, 0, 0, 0, true);
    $CHRISTINE =  json_decode($DOCTOR_STRANGE["anas"]["data"], true);
    // file_put_contents("hext.txt", $DOCTOR_STRANGE["anas"]["data"]);
    if (isset($CHRISTINE["js"]["token"])) {
        return json_encode(array("anas" => array("data" =>
        array(
            "Author" => "DOCTOR_STRANGE",
